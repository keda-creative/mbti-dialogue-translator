...
function formatBaseContext(
  request: AnalyzeIntentRequest | TranslationRequest
): string {
  return [
...
已确认意图卡：
${JSON.stringify(request.intentCards, null, 2)}

澄清回答：
${JSON.stringify(request.clarificationAnswers, null, 2)}

primary 意图：
${JSON.stringify(primaryIntents, null, 2)}

sensitive 意图：
${JSON.stringify(sensitiveIntents, null, 2)}
